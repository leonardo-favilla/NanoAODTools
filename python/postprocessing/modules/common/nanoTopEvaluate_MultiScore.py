...
models["base2"]         = tf.keras.models.load_model(f"{path_to_model_folder}/model_base2.h5")
...
class nanoTopevaluate_MultiScore(Module):
    ...
    def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
        ...
        for key in keys:
            self.out.branch(f"TopHighPt_score_{key}", "F", lenVar="nTopHighPt")
    ...
    def analyze(self, event):
        ...
        if len(tophighpt)!=0:
            ...
            for key in keys:
                scores[key] = models[key].predict({"fatjet": fj_dnn, "jet": jets_dnn, "top": mass_dnn}).flatten().tolist()
        else:
            top_score2  = []
            for key in keys:
                scores[key] = []
        ...
        for key in keys:
            self.out.fillBranch(f"TopHighPt_score_{key}", scores[key])
        ...
        for i, top in enumerate(toplowpt):
            ...
            jets_dnn = fill_jets(jets_dnn, j0, j1, j2, sumjet, fj.Phi(), fj.Eta(), i)
        if len(toplowpt)!=0:
            top_score_DNN = models["scoreDNN"].predict({"jet0": jets_dnn[:,0,:-2], "jet1": jets_dnn[:,1,:-2], "jet2": jets_dnn[:,2,:-2]}).flatten().tolist()
        else:
            top_score_DNN = []
