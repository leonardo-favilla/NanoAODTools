...
for label in utilities.keys():
    for c in utilities[label].keys():
        ...
        for path_to_rfile, path_to_rHisto in zip(utilities[label][c]["rFiles"], utilities[label][c]["rHistos"]):
            print(f"\tRunning:\t{path_to_rfile}\tSaving histos to:\t{path_to_rHisto}")
            os.system(f"python3 FillHistos.py -dataset {c} -path_to_rfile {path_to_rfile} -save_graphics {save_graphics} -path_to_graphics_folder {'/'.join(path_to_rHisto.split('/')[:-1])} -rhistos_filename {path_to_rHisto.split('/')[-1]}")
